$(document).ready(function(){
	const MODIFY_SUCCESS_TIP="#modifySuccessTip";
	const MODIFY_FAIL_TIP="#modifyFailTip";
		...
		$.ajax({
			...
			success:function(result){
				if(RESULT_SUCCESS == result){
					$(J_USERNAME).addClass(CSS_HAS_ERROR);
					$(J_USERNAME_TIP).text(USERNAME_NON_EXISTED);
					$(J_USERNAME_TIP).addClass(CSS_ALERT_ERROR);
	...
	$(J_SUBMIT).click(function(){
		...
		$.ajax({
			type:METHOD_POST,
			url:USERINFO_MANAGER_GET_PASSWORD_ACTION,
			data:{'username':$(J_USERNAME).val(),'password':$(J_PASSWORD).val(),'email':$(J_EMAIL).val()},
			dataType:'text',
			success:function(result){
				if(RESULT_SUCCESS == result){
					$(MODIFY_SUCCESS_TIP).removeClass(CSS_HIDE);
					setTimeout(function(){
						window.location=USERINFO_MANAGER_INDEX_PAGE;
					},1000);
				}else{
					$(MODIFY_FAIL_TIP).removeClass(CSS_HIDE);
					$(J_USERNAME).val("");
					$(J_EMAIL).val("");
					$(J_USERNAME).focus();
					setTimeout(function(){
						$(MODIFY_FAIL_TIP).addClass(CSS_HIDE);
					},1500);
				}
			}
		});
